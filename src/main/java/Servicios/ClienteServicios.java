...
public class ClienteServicios {
    ...
    public static JsonResponse<Cliente> queryCliente(JsonObject jsonObject) throws SQLException {
        String id_cliente = jsonObject.get(Constantes.JSON_KEY_CLIENT).getAsString();
        String argumentos;
        if (id_cliente == null) {
            argumentos = " WHERE " + CheckMobileTables.CLIENTE.ESTADO + " = " + " 'A'"
                    + CheckMobileTables.CLIENTE.ID_EMPRESA + " = " + Constantes.ID_EMPRESA;
        } else {
            argumentos = " WHERE " + CheckMobileTables.CLIENTE.ID_CLIENTE
